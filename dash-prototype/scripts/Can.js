    ...
    channel.addListener("onMessage", function(msg) {
      switch (msg["id"]) {
        ...
        case rpmAddr:
          ...
          //add rpm to mph here Aniruddh//
          
          //curr_rpm=
          if (curr_rpm < 8050 && curr_rpm > 90) {
            //rpm.textContent = curr_rpm.toString();
            debugrpm.textContent = curr_rpm.toString();
            //added rpm to mph here Aniruddh//
            mph.textContent=Math.round(rpmToMph(curr_rpm)).toString();
  
            // update rpm bar on dash
            if (curr_rpm < RPM_PACE) {
              rpmBar.set((1.0 / 3.0) * (curr_rpm / RPM_PACE));
              //rpmBar.set(Math.round(rpmToMph(curr_rpm)).toString());
              //make calculations in rpm set bar( i beliieve the formula can be written inside
            } else {
        ...
        case bmsTempsAddr:
          ...
          curr_mincelltemp = ((msg["data"][3] << 8) + msg["data"][2]);
           ...
           tempdata.push(curr_mctemp);//line added by aniruddh
